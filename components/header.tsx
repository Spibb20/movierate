...
const navLinks = [
  { href: "/", label: "Нүүр" },
  { href: "/movies", label: "Төрлөөр хайх", icon: Grid3X3 },
]
        ...
        <div ref={searchRef} className="relative hidden flex-1 md:block">
          ...
          {searchOpen && searchQuery.length > 0 && searchResults.length === 0 && (
            <div className="absolute left-0 top-full z-50 mt-2 w-full max-w-sm rounded-xl border border-border bg-card p-4 shadow-lg">
              <p className="text-center text-sm text-muted-foreground">Илэрц олдсонгүй</p>
            </div>
          )}
        </div>
            ...
            <div className="mt-2 flex flex-col overflow-hidden rounded-lg border border-border bg-card">
              {searchResults.map((movie) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">{movie.title}</p>
                    <p className="truncate text-xs text-muted-foreground">{movie.year}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <nav className="flex flex-col gap-1">
            ...
            <div className="mt-2 flex flex-col gap-2 border-t border-border pt-3">
              ...
              <Link href="/profile" onClick={() => setMobileOpen(false)}>
                <Button size="sm" className="w-full justify-start gap-1.5 bg-accent text-accent-foreground hover:bg-accent/90">
                  <User className="h-3.5 w-3.5" />
                  Профайл
                </Button>
              </Link>
