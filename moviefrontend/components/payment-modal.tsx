...
import rentalConfig from "@/data/rental-config.json";
...
export function PaymentModal({
  ...
  const methodIconMap = { Smartphone, CreditCard, Building2 } as const;
  const methods = paymentMethodsData.map((paymentMethod) => ({
    ...paymentMethod,
    icon: methodIconMap[paymentMethod.icon as keyof typeof methodIconMap],
  }));
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {movie.title} - {rentalConfig.durationDays} хоногийн эрх
            </p>
          </div>
          <button
            onClick={handleClose}
              ...
              {methods.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    onClick={() => {
                      setMethod(m.id as Exclude<PaymentMethod, null>);
                      setStep("details");
                    }}
              ...
              {method === "socialpay" && (
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    SocialPay апп руу нэхэмжлэл илгээгдэнэ. Апп дотроосоо
                    баталгаажуулна уу.
                  </p>
                  <Button
                    onClick={handlePay}
                    ...
                    <div className="flex flex-col gap-1.5 text-sm text-muted-foreground">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-foreground">
                          {rentalConfig.bank.name}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Данс:</span>
                        <span className="font-mono font-medium text-foreground">
                          {rentalConfig.bank.account}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Хүлээн авагч:</span>
                        <span className="font-medium text-foreground">
                          {rentalConfig.bank.receiver}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Дүн:</span>
            ...
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Кино {rentalConfig.durationDays} хоногийн турш үзэх эрхтэй
                  боллоо.
                </p>
              </div>
              <div className="flex items-center gap-1.5 rounded-lg bg-secondary/60 px-3 py-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {rentalConfig.durationDays} өдөр үлдсэн
                </span>
              </div>
              <Button
                onClick={handleDone}
